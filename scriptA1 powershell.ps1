...
function Manage-Users {
    ...
    switch ($choice) {
        1 {
            ...
            # Créez une nouvelle session SSH et stockez-la dans une variable
            $session = New-SSHSession -ComputerName "hostname_or_ip" -Credential (Get-Credential)
            
            # Utilisez la session SSH créée pour exécuter une commande à distance
            $output = Invoke-SSHCommand -SessionId $session.SessionId -Command $command
            
            # Fermez la session SSH une fois les commandes terminées
        ...
        2 {
            ...
            Write-Log "Suppression de l'utilisateur $username"
        }
        3 {
            return
        }
