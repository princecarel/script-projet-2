...
function Show-Menu {
    Write-Host "1. Gérer les utilisateurs de WINSERV"
    Write-Host "2. Gérer les ordinateurs"
    Write-Host "3. Quitter"
}

# Fonction pour gérer les utilisateurs de WINSERV
function Manage-Users {
    ...
    switch ($choice) {
        1 {
            ...
            $session = New-PSSession -ComputerName "WINSERV" -Authentication Kerberos -Credential (Get-Credential)
            Invoke-Command -Session $session -ScriptBlock { param($command) Invoke-Expression $command } -ArgumentList $command
            Write-Log "Création de l'utilisateur $username sur WINSERV"
        ...
        2 {
            ...
            $session = New-PSSession -ComputerName "WINSERV" -Authentication Kerberos -Credential (Get-Credential)
            Invoke-Command -Session $session -ScriptBlock { param($command) Invoke-Expression $command } -ArgumentList $command
            Write-Log "Suppression de l'utilisateur $username sur WINSERV"
            Remove-PSSession -Session $session
        }
        3 {
            return
